...
import '../../../.storybook/stylefix';

# BaseSelect

Является полем ввода. Представляет собой выпадающий список и предназначен для выбора одного или несколько значений из множества. Представлен тэгом `<base-select>`.
...
| value        | Предустановленное значение или массив значений (если `multiple=true`) компонента.                                                  | Любое или Массив значений      |
...
| multiple     | Позволяет одновременно выбирать сразу несколько элементов списка. Рукомендуется применять вместе со свойством `autoClose=false`.   | Логическое                     |
...
| data-auto-close | Атрибут свойства "autoClose".                                                                         | "false"                     |
| value           | Предустановленное значение компонента. Данный html-атрибут используется в режиме одиночного значения. | Строка                      |
| selected        | Заранее устанавливает определённые пункты списка (элементы `slot="item"`) выделенными в режиме `multiple=true`. Нужен для предустановки значений в разметке, не используя атрибут `value`. | 
...
| Название  | Описание                             |
|-----------|--------------------------------------|
| input     | Событие ввода данных компонента.     |
| change    | Событие изменения данных компонента. |
...
Выпадающий список с возможностью выбора несколько выриантов:

<Canvas>
  <base-select multiple data-auto-close="false" label="Multiple select">
    <div slot="item" value="1" data-visible-value="Один">Один (1)</div>
    <div slot="item" value="2" data-visible-value="Два" selected>Два (2)</div>
    <div slot="item" value="3" data-visible-value="Три" selected>Три (3)</div>
    <div slot="item" value="4" data-visible-value="Четыре">Четыре (4)</div>
  </base-select>
</Canvas>
...
dark
code={`
<base-select multiple label="Multiple select">
  <div slot="item" value="1" data-visible-value="Один">Один (1)</div>
  <div slot="item" value="2" data-visible-value="Два" selected>Два (2)</div>
  <div slot="item" value="3" data-visible-value="Три" selected>Три (3)</div>
  <div slot="item" value="4" data-visible-value="Четыре">Четыре (4)</div>
